...
The problem with the "separation of duties" approach to solving all your stability issues is that everyone can make mistakes. A job title doesn't imply infallibility. Every sysadmin who thinks that their title means they won't make a mistake will tell me a story over a couple of pints about when they totally messed up production.
...
No matter how much you try to block the "how", you should focus on the "what". Consider the outcome, not the mechanism that leads to that outcome. 
...
InSpec is compliance as code. We write controls that test for the state of the system and report back on this state. It's also pretty easy to read, as it's written in a spec-style language. Consider the difference between these two ways of checking the SSH protocol version on a system:
...
(All code for this post can be found at [github.com/mattstratton/sysadvent-2017](https://www.github.com/mattstratton/sysadvent-2017))
...
In this case, I've created a Jenkins pipeline that responds to changes in the cookbook's GitHub repo. The [Jenkinsfile](https://github.com/mattstratton/sa2017-app/blob/master/Jenkinsfile) defines the following stages:
...
1. Compliance - Runs `kitchen test`, but includes our compliance profile, loaded with a `git` URL.

In order to accomplish the `Compliance` stage, we generate a new `kitchen.yml` file, which includes the path to the test we want to include. There are other ways to accomplish this, and it will depend on how you build your pipelines. But hopefully, this illustrates one way it can be done.
